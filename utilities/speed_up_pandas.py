...
def panda_mem_usage(df, detail='full'):
    ...
    if detail == 'full':
        ...
        for index, row in df1.iterrows():
            print('{:<15} {:<15} {:>15,.0f} {:>8,.1f} {:>8,.2f}'.format(row['col'], str(row['type']), row['bytes'],
                                                                        row['bytes'] / 1024 ** 2,
                                                                        row['bytes'] / 1024 ** 3))

        print(
            '\nTotal: {:,.0f} Rows, {:,.0f} Bytes, {:,.1f} MBs, {:,.2f} GBs\n'.format(len(df), total, total / 1024 ** 2,
                                                                                      total / 1024 ** 3))
...
def read_file(fn, compression='', sql=''):
    ...
    if fn_ext == '.csv' or fn_ext == '.zip':
        ...
        df = pd.read_pickle(fn,
                            compression=compression).copy()  # copy added because of some trouble with categories not fully read by mem util on first pass
...
def create_test_dataframe(start_date, end_date, num_symbols, squeeze=True, out=''):
    ...
    df['dupe_num'] = df['dupe_num'].astype(str).str.zfill(
        len(str(num_symbols)))  # pad with 0's based on num_symbols length
